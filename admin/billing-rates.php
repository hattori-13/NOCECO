...
<body class="bg-noceco-bg text-noceco-text min-h-screen pb-20">
    ...
    <?php if ($generatedInvoice): 
        ...
        // FUTURE PENALTY CALCULATION (If paid after due date)
        $current_penalty = $absolute_grand_total * 0.05;
        $amount_after_due = $absolute_grand_total + $current_penalty;
    ?>
    ...
    <div id="thermal-receipt" class="hidden print:block p-1">
        ...
        <table style="width:100%; font-size:9px; border-collapse: collapse; margin-top: 3px;">
            ...
            <?php foreach ($generatedInvoice['line_items'] as $item): 
                if ($item['amt'] == 0 || $item['is_vat_item']) continue;
            ?>
            <tr>
        ...
        <!-- THERMAL VAT SECTION -->
        <div class="thermal-divider" style="margin-top: 6px;"></div>
        <div class="thermal-text" style="font-weight: bold; margin-bottom: 2px;">Vat Detail:</div>
        <table style="width:100%; font-size:9px; border-collapse: collapse;">
            ...
            ?>
            <tr>
                <td style="width:40%; text-align:left;"><?php echo substr($item['desc'], 0, 15); ?></td>
                <td style="width:35%; text-align:center;">x <?php echo number_format($item['rate'], 4) . $rateSuffix; ?></td>
                <td style="width:25%; text-align:right;"><?php echo number_format($item['amt'], 2); ?></td>
            </tr>
            <?php endforeach; ?>
        </table>
        ...
        <div class="thermal-text" style="display:flex; justify-content:space-between; margin-bottom: 3px;">
            <span>TOTAL VAT:</span>
            <span><?php echo number_format($generatedInvoice['explicit_vat_total'], 2); ?></span>
        </div>
        <div class="thermal-text" style="display:flex; justify-content:space-between; font-weight: bold; font-size: 11px;">
            <span>AMOUNT DUE:</span>
            <span>Php <?php echo number_format($absolute_grand_total, 2); ?></span>
        </div>
        <div class="thermal-text" style="display:flex; justify-content:space-between;">
            <span>DUE DATE:</span>
            <span><?php echo date('m/d/Y', strtotime($generatedInvoice['due_date'])); ?></span>
        </div>
        <div class="thermal-text" style="display:flex; justify-content:space-between;">
            <span>PENALTY @5%:</span>
            <span>Php <?php echo number_format($current_penalty, 2); ?></span>
        </div>
        <div class="thermal-text" style="display:flex; justify-content:space-between; font-weight: bold; font-size: 11px; margin-top: 3px; padding-top: 3px; border-top: 1px dashed black;">
            <span>AFTER DUE:</span>
            <span>Php <?php echo number_format($amount_after_due, 2); ?></span>
        </div>
        <div class="thermal-divider"></div>
        ...
        <?php if ($generatedInvoice): ?>
            ...
            <div class="bg-white rounded-[20px] shadow-apple border border-gray-200 overflow-hidden mb-6 font-mono text-sm relative">
                ...
                <div class="p-6 pt-8">
                    ...
                    <!-- ENHANCED DIGITAL BREAKDOWN: Segregates normal vs VAT -->
                    <div class="mt-4 pt-4 border-t border-dashed border-gray-300">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="flex justify-between text-[11px] text-gray-700 items-center">
                                ...
                                <span class="w-1/4 text-center text-gray-400 text-[10px]">
                                    <?php echo $item['type'] === 'Per_KWH' ? number_format($item['rate'], 4).'/kwh' : 'Fixed'; ?>
                                </span>
                        ...
                        <p class="text-[11px] mb-2 mt-4 font-bold border-t border-dashed border-gray-300 pt-3 text-gray-900">Vat Detail:</p>
                        <div class="space-y-1 text-gray-600 pl-2">
                            <?php foreach ($generatedInvoice['line_items'] as $item): 
                                if ($item['amt'] == 0 || !$item['is_vat_item']) continue; 
                                $rateSuffix = $item['type'] === 'Per_KWH' ? '/kwh' : '/Cust';
                            ?>
                            <div class="flex justify-between text-[11px] leading-tight items-center">
                                <span class="w-1/2 truncate pr-1 font-medium"><?php echo htmlspecialchars($item['desc']); ?></span>
                                <span class="w-1/4 text-center">x <?php echo number_format($item['rate'], 4) . $rateSuffix; ?></span>
                    ...
                    <div class="mt-5 pt-4 border-t-2 border-dashed border-gray-300">
                        <p class="text-center text-xs font-bold mb-3 tracking-widest uppercase text-gray-500">Summary</p>
                        <div class="flex justify-between text-[11px] mb-1 font-bold text-gray-700">
                            <span>TOTAL Value Added TAX 12%</span>
                            <span><?php echo number_format($generatedInvoice['explicit_vat_total'], 2); ?></span>
                        </div>
                        <div class="flex justify-between font-bold text-sm mb-1 pt-2 border-t border-gray-200 text-gray-800">
                            <span>TOTAL SALES</span>
                            <span>Php <?php echo number_format(($generatedInvoice['grand_total'] - $generatedInvoice['explicit_vat_total']), 2); ?></span>
                        </div>
                        
                        <div class="flex justify-between font-black text-xl pt-2 text-gray-900 mt-2">
                            <span>AMOUNT DUE</span>
                            <span>Php <?php echo number_format($absolute_grand_total, 2); ?></span>
                        </div>
                        
                        <!-- ADDED DUE DATE & PENALTIES TO DIGITAL DISPLAY -->
                        <div class="flex justify-between text-[11px] font-bold text-gray-500 mt-3">
                            <span>DUE DATE</span>
                            <span><?php echo date('M d, Y', strtotime($generatedInvoice['due_date'])); ?></span>
                        </div>
                        <div class="flex justify-between text-[11px] font-bold text-red-400 mt-1">
                            <span>PENALTY @5%</span>
                            <span>Php <?php echo number_format($current_penalty, 2); ?></span>
                        </div>
                        <div class="flex justify-between font-bold text-sm pt-2 border-t border-gray-200 text-red-600 mt-2">
                            <span>AMOUNT AFTER DUE</span>
                            <span>Php <?php echo number_format($amount_after_due, 2); ?></span>
                        </div>
                    </div>
